...
def upload():
    response.headers['Access-Control-Allow-Origin'] = "*"
    image = db.image.imagefile.store(request.vars.imagefile.file, request.vars.imagefile.filename)
    id = db.image.insert(   address=request.vars.address,
...
def getimagelist():
    ...
    for image in images:
        ...
        imageList[image.id] = dict( id=image.id,
